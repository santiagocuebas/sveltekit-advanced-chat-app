					...
					{#if content.length > 1}
						<div>
							{#each content as image (image)}
								<picture>
									<img src={image} alt={_id} role='none' on:click={handleImage}>
								</picture>
							{/each}
						</div>
					{:else}
						{#if isVideo(content[0])}
							<video controls src={content[0]}>
								<track kind="captions"/>
							</video>
						{:else if isAudio(content[0])}
							<audio controls src={content[0]}></audio>
						{:else}
							<picture>
								<img src={content[0]} alt={_id} role='none' on:click={handleImage}>
							</picture>
						{/if}
					{/if}
